...
resource "kubernetes_secret_v1" "auth_db_secret_host" {
  ...
  data = {
    DATABASE_URL = "postgres://togglemaster_admin:${var.rds_password}@${var.db_auth_endpoint}"
  }
...
resource "kubernetes_secret_v1" "targeting_db_secret_host" {
  ...
  data = {
    DATABASE_URL = "postgres://togglemaster_admin:${var.rds_password}@${var.db_targeting_endpoint}"
  }
